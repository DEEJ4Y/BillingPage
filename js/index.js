    ...
    function displayAddedItem(applianceName){
        ...
        $(".cartItemsBorder").before("<div class='cart-item-row'><span style='float: left; margin-right= 2rem;'>" + cartItems.length + ". " + appliance + "</span><span style='float: right;'>" + price + " <button class='btn btn-sm btn-outline-danger remove-button' style='float: right;z-index=-10' id='remove-button-" + cartItems.length + "'>Remove</button></span> </div>");
        $(".remove-button").click(removeFromCart);
    }

    function removeFromCart(){
        var cartPosition = ($(this).attr("id")).slice(14, ($(this).attr("id")).length);
        console.log(cartPosition);
        $(".cart-item-row:contains('"+ cartPosition +".')").hide();
        for(var i = cartPosition - 1; i < cartItems.length - 1; i++){
